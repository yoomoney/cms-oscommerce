...
use YandexMoney\Updater\GitHubConnector;
...
class Updater
{
    ...
    public function getVersionInfo($force = false)
    {
        ...
        $result = array();
        if (version_compare($versionInfo['version'], \Yandex_Money::MODULE_VERSION) > 0) {
            $result['new_version_available'] = true;
            $result['changelog'] = $this->getChangeLog(\Yandex_Money::MODULE_VERSION, $versionInfo['version']);
            $result['newVersion'] = $versionInfo['version'];
        } else {
            $result['new_version_available'] = false;
            $result['changelog'] = '';
            $result['newVersion'] = \Yandex_Money::MODULE_VERSION;
        }
        $result['currentVersion'] = \Yandex_Money::MODULE_VERSION;
